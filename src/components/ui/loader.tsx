    ...
    <div className="flex items-center gap-2">
      {[0, 1, 2].map((i) => (
        <motion.div
          ...
          className="h-4 w-4 rounded-full border border-neutral-300 bg-gradient-to-t from-[#008DD2] to-[#fff]/50"
        />
      ))}
    </div>
  );
...
// Loader Three (animated SVG path)
...
// Loader Four (glitchy text)
export const LoaderFour = ({ text = "Loading..." }: { text?: string }) => {
  return (
    <div className="relative font-bold text-black [perspective:1000px] dark:text-white">
      <motion.span
        animate={{
          skewX: [0, -40, 0], //  changed from skew -> skewX
          scaleX: [1, 2, 1],
        }}
        transition={{
          duration: 0.05,
