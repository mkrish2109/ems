...
export const AddMemberForm: React.FC = () => {
  ...
  const closePopup = () => {
    setShowPopup(false);
    // If it was a success popup, redirect to dashboard
  ...
  const handleInvite = async () => {
    ...
    if (typeof err === 'object' && err !== null && 'errors' in err) {
      ...
      // Map API errors to form validation errors
      if (apiErrors.email) {
        validationErrors.email = apiErrors.email[0];
      }
      
      if (apiErrors.mobile) {
        validationErrors.mobile = apiErrors.mobile[0];
      }
      
      if (apiErrors.user_name) {
        validationErrors.user_name = apiErrors.user_name[0];
      }
      
      if (apiErrors.relation) {
        validationErrors.relation = apiErrors.relation[0];
      }
      
      if (apiErrors.role) {
        validationErrors.role = apiErrors.role[0];
      }
      
      // Set popup message to first error
